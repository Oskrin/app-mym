app.controller('empleadosController', function ($scope, $route, loaddatosSRI) {
	...
	jQuery(function($) {
		...
		// tabs
		$( "#tabEmpleado" ).click(function(event) {
			event.preventDefault();  
		});	

		$("#tabEmpleado").on('shown.bs.tab', function(e) {
		...
		// stilo cargo
		$('#select_cargo').multiselect({
		  	nonSelectedText: 'Seleccione los Cargos',
		...
		// stilo cargo
		$('#select_servicio').multiselect({
		  	nonSelectedText: 'Seleccione los Servicios',
		  	enableFiltering: true,
		  	enableCaseInsensitiveFiltering: true,
		  	buttonWidth:'100%'
		});
			...
			$.ajax({
	            ...
	            success: function(data) {
	                ...
	                if (val == 1) {
	                    ...
	                    $.gritter.add({
							title: 'Error... El Empleado ya se encuentra Registrado',
		...
		function llenar_select_cargo() {
			$.ajax({
				...
				success: function(data) {
					$('#select_cargo').html(data).multiselect('rebuild');
				}
			});
		}
		// fin

		// llenar combo servicio
		function llenar_select_servicio() {
			$.ajax({
				...
				success: function(data) {
					$('#select_servicio').html(data).multiselect('rebuild');
				}
			});
		}
		// fin
    	...
    	$("#logo").attr("src", "data/empleados/fotos/defaul.jpg");
    	$('#btn_4').attr('disabled', true); 
    	// fin

		// guardar formulario
		$('#btn_0').click(function() {
			var respuesta = $('#form_empleados').valid();
			
			if (respuesta == true) {
				$('#btn_0').attr('disabled', true); 
				var formData = new FormData(document.getElementById("form_empleados"));
		...
		jQuery(function($) {
		    ...
		    jQuery(grid_selector).jqGrid({	        
		        datatype: "xml",
		        url: 'data/empleados/xml_empleados.php',        
		        colNames: ['ID','IDENTIFICACIÓN','NOMBRES COMPLETOS','TELÉFONO','CELULAR','CIUDAD','DIRECCIÓN','CORREO','FOTO','OBSERVACIONES','','ESTADO'],
		        colModel:[      
		        ...
		        ondblClickRow: function(rowid) {     	            	            
	            	...
	            	$('#id').val(ret.id);
	            	$("#logo").attr("src", "data/empleados/fotos/"+ ret.foto);
		            ...
		            $.ajax({
						...
						dataType: 'json',
						success: function(data) {
							$("#select_provincia").select2('val', data.id_provincia).trigger("change");
							$("#select_ciudad").select2('val', ret.id_ciudad).trigger("change");
						}
					});

	            	$('#direccion').val(ret.direccion);
	            	$("#select_cargo").select2('val', ret.id_cargo).trigger("change");
	            	$("#select_servicio").select2('val', ret.id_servicio).trigger("change");
		        ...
		        editurl: "data/empleados/app.php",
