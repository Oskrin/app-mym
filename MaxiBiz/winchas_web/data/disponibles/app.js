app.controller('disponiblesController', function ($scope) {
	...
	jQuery(function($) {
		...
		function llenar_tabla() {
			var estado = "<span class='label label-success arrowed-in arrowed-in-right'>Disponible</span>";
			var estado2 = "<span class='label label-danger arrowed-in arrowed-in-right'>Ocupado</span>";

			$.ajax({
				url: 'data/disponibles/app.php',
				type: 'post',
				...
				success: function(response) { 
					if (response != null) {
						...
						for (var i = 0; i < response.length; i++) {
							var dir = response[i]['imagen'];
							var imagen = "<img  src='data/winchas/fotos/"+dir+"' width='100' height='95'>";

							if(response[i]['estado'] == '1') {
								tabla.row.add([
							            imagen,
							            response[i]['modelo'],
							            response[i]['responsable'],
							            estado
				                ]).draw(false); 
				            } else {
